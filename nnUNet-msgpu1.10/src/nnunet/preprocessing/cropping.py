...
def get_case_identifier(case):
    """get case identifier"""
    case_identifier = case[0].split("/")[-1].replace("_0000.nii.gz", "")
...
class ImageCropper():
    ...
    def load_crop_save(self, case, case_identifier, overwrite_existing=False):
        """load crop save case"""
        try:
            print(f"Processing: {case_identifier}")
            if overwrite_existing \
                ...
                print(f"Successfully processed: {case_identifier}")
                return True
            else:
                print(f"Skipping (already exists): {case_identifier}")
                return True
        except Exception as e:
            print(f"ERROR in {case_identifier}: {str(e)}")
            import traceback
            traceback.print_exc()
            # 不再抛出异常，而是返回False，这样其他case可以继续处理
            return False
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        print(f"\n{'='*60}")
        print(f"Starting cropping for {len(list_of_args)} cases with {self.num_threads} threads")
        print(f"{'='*60}\n")

        p = Pool(self.num_threads)
        results = p.starmap(self.load_crop_save, list_of_args)
        p.close()
        p.join()
        ...
        if failed_count > 0:
            print(f"  WARNING: {failed_count} cases failed to process!")
        print(f"{'='*60}\n")
