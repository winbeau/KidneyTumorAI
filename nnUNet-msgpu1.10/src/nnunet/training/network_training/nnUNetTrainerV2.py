...
# from mindspore._checkparam import Validator as validator  # MindSpore 2.6.0兼容性修复
import numpy as np
...
class WithEvalCell(nn.Cell):
    ...
    def __init__(self, network, loss_fn, add_cast_fp32=False):
        ...
        self._loss_fn = loss_fn
        # self.add_cast_fp32 = validator.check_value_type("add_cast_fp32", add_cast_fp32, [bool], self.cls_name)  # Mind Spore 2.6.0兼容性修复
        self.add_cast_fp32 = add_cast_fp32  # 直接赋值，跳过类型检查
...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def one_hot2D(self, labels, num_classes=3):
        ...
        # Add error handling for tensor conversion to prevent CUDA stream failures
        max_retries = 3
        for retry in range(max_retries):
            try:
                ...
                if "CUDA stream" in str(e) and retry < max_retries - 1:
                    print(f"CUDA stream error in label conversion, retrying... ({retry + 1}/{max_retries})")
                    # Small delay to allow CUDA resources to free up
                    import time
                    time.sleep(0.1)
                    continue
                else:
        ...
        # Add error handling for tensor creation to prevent CUDA stream failures
        max_retries = 3
        for retry in range(max_retries):
            try:
                ...
                if "CUDA stream" in str(e) and retry < max_retries - 1:
                    print(f"CUDA stream error in tensor creation (2D), retrying... ({retry + 1}/{max_retries})")
                    mindspore.ops.hal.sync_stream()
                    continue
                else:
                    raise

        num_classes, on_value, off_value = num_classes, on_value, off_value
    ...
    def one_hot3D(self, labels, num_classes=3):
        ...
        for retry in range(max_retries):
            try:
                ...
                if "CUDA stream" in str(e) and retry < max_retries - 1:
                    print(f"CUDA stream error in tensor creation, retrying... ({retry + 1}/{max_retries})")
                    # Small delay to allow CUDA resources to free up
                    import time
                    time.sleep(0.1)
                    continue
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=True):
        ...
        # Add retry mechanism for tensor conversions to handle CUDA stream errors
        max_retries = 3
        for retry in range(max_retries):
            try:
                ...
                if "CUDA stream" in str(e) and retry < max_retries - 1:
                    print(f"CUDA stream error, retrying... ({retry + 1}/{max_retries})")
                    # Small delay to allow CUDA resources to free up
                    import time
                    time.sleep(0.1)
                    continue
    ...
    def on_epoch_end(self):
        ...
        if self.epoch == 100:
            if self.all_val_eval_metrics[-1] == 0:
                # For MindSpore, we need to recreate the optimizer with different beta1 (momentum)
                # since MindSpore optimizers don't have param_groups like PyTorch
                self.optimizer = nn.Adam(
                    params=self.network.trainable_params(),
                    learning_rate=self.lr[self.epoch] if self.epoch < len(self.lr) else self.initial_lr,
                    weight_decay=0.0005,
                    loss_scale=1024.0,
                    beta1=0.95  # Reduce momentum from default 0.9 to 0.95 (equivalent to PyTorch momentum)
                )
