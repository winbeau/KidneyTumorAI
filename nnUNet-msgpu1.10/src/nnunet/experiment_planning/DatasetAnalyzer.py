...
class DatasetAnalyzer():
    ...
    def analyse_segmentations(self):
        ...
        if self.overwrite or not isfile(self.props_per_case_file):
            print(f"\nAnalyzing segmentations for {len(self.patient_identifiers)} cases...")
            ...
            print(f"Segmentation analysis completed for {len(props_per_patient)} cases")
        else:
            print("Loading existing segmentation analysis...")
    ...
    def collect_intensity_properties(self, num_modalities):
        ...
        if self.overwrite or not isfile(self.intensityproperties_file):
            print(f"\nCollecting intensity properties for {num_modalities} modalities...")
            p = Pool(self.num_processes)

            results = OrderedDict()
            try:
                for mod_id in range(num_modalities):
                    print(f"  Processing modality {mod_id + 1}/{num_modalities}...")
                ...
                print("Intensity properties collection completed")
            except Exception as e:
                print(f"ERROR during intensity properties collection: {str(e)}")
                p.terminate()
                p.join()
                raise e
        else:
            print("Loading existing intensity properties...")
